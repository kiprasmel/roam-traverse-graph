...
const findPublicPages = (
	...
	const partlyPublicPages = somePages
		...
		.map((page) => {
			...
			const linkedReferencesMatchers = [/\#\w+/, /\[\[\w+\]\]/, /* /\w+\:\:/ */ /* disabling attributes intentionally, needs work-around to create proper one */ ];

			const findLinkedReferencesForPage = (p) => findMatchesForMultiple(
			...
			const matchAtLeastOne = (testee, matchers = linkedReferencesMatchers) => matchers.some(matcher => matcher.test(testee))

			/** @type { (page: import("./types").PageOrBlock) => boolean } */
			const doesPageHaveAtLeastOneLinkedReference = (p) => "title" in p ? matchAtLeastOne(p.title) : "string" in p ? matchAtLeastOne(p.string) : false;
